...
2. 21.94
<br>
3. Currencies, stock exchanges, oil and commodities.  They are fast traded items.  
<br>
4. First see crisis, then see bull data.  It shows that the principal components become more important in bear markets (i.e. have more impact), and less important in bull markets (i.e. have less impact).
...
Crisis_Explained_Table_results<-PCA(crisis_data, graph=FALSE)
Crisis_Explained_Table<-cbind(paste("component",1:ncol(crisis_data),sep=" "),Crisis_Explained_Table_results$eig)
Crisis_Explained_Table<-as.data.frame(Crisis_Explained_Table)
colnames(Crisis_Explained_Table)<-c("Component","Eigenvalue", "Percentage_of_explained_variance", "Cumulative_percentage_of_explained_variance")

show_crisis_data = data.frame(Crisis_Explained_Table)
m2<-gvisTable(show_crisis_data,options=list(width=1920, height=min(400,27*(nrow(show_crisis_data)+1)),allowHTML=TRUE,page='disable'),formats=list(Eigenvalue="#.##",Percentage_of_explained_variance="#.##",Cumulative_percentage_of_explained_variance="#.##"))
print(m2,'chart')
...
Bull_Explained_Table_results<-PCA(bull_data, graph=FALSE)
Bull_Explained_Table<-cbind(paste("component",1:ncol(bull_data),sep=" "),Bull_Explained_Table_results$eig)
Bull_Explained_Table<-as.data.frame(Bull_Explained_Table)
colnames(Bull_Explained_Table)<-c("Component","Eigenvalue", "Percentage_of_explained_variance", "Cumulative_percentage_of_explained_variance")

show_bull_data = data.frame(Bull_Explained_Table)
m2<-gvisTable(show_bull_data,options=list(width=1920, height=min(400,27*(nrow(show_bull_data)+1)),allowHTML=TRUE,page='disable'),formats=list(Eigenvalue="#.##",Percentage_of_explained_variance="#.##",Cumulative_percentage_of_explained_variance="#.##"))
print(m2,'chart')
...
Variance_Explained_Table_results<-PCA(my_data, graph=FALSE)
Variance_Explained_Table<-cbind(paste("component",1:ncol(my_data),sep=" "),Variance_Explained_Table_results$eig)
Variance_Explained_Table<-as.data.frame(Variance_Explained_Table)
colnames(Variance_Explained_Table)<-c("Component","Eigenvalue", "Percentage_of_explained_variance", "Cumulative_percentage_of_explained_variance")
```
